#produce content displayed through ui
  ...
  #-----------------------------------
    ...
    output$timelineText <- renderText({
      ...
      ifelse(which(tbDays$Date == Sys.Date())<=100,
        paste0("INSERT YOUR MAYOR took office on 12/8/20. Today is day ", 
            which(tbDays$Date == Sys.Date()),
            "."
          ),
        #For when the 100 days are complete
        paste0("The 100 Days of Action began when INSERT YOUR MAYOR took office ",
    ...
    output$progressLegend <- renderUI({
      div(class="legend", 
          div(style=paste0("background:", complete_color, ";")), p("Complete"),
          div(style=paste0("background:", progress_color, ";")), p("In progress"),
          div(style=paste0("background:", notstarted_color, ";")), p("Not yet started")
      )
    })
  ...
  output$timelineLegend <- renderUI({
    div(class="legend", 
      div(style=paste0("background:", past_color, ";")), p("Past"),
      div(style=paste0("background:", current_color, ";")), p("Current"),
      div(style=paste0("background:", remaining_color, ";")), p("Remaining")
    )
  })
  ...
  output$downloadActions <- downloadHandler(
    filename = function() {
      paste(Sys.Date(), "-transition-tracker-actions.csv", sep="")
    },
